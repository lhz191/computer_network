...
// 客户端实现
void udpClient(const char* clientIP, const char* serverIP, uint16_t serverPort, uint16_t clientPort) {
    // 初始化Winsock
    WSADATA wsaData;
    ...
    while (true) {
        ...
        cout << "================进行文件传输===================" << endl;
        // 动态分配的 buffer 已不必要，改为逐块读取传输
        cout << "文件读取成功，正在进行文件传输..." << endl;
        // 记录文件传输开始时间
        auto startTime = getCurrentTime();
        char data[BUFFER_SIZE];
        while (!file.eof()) { // 检查是否到达文件末尾
            file.read(data, BUFFER_SIZE); // 读取固定大小的数据块
            int bytesRead = file.gcount(); // 获取实际读取的字节数

            if (bytesRead > 0) {
                // 创建 UDP 数据包
                UDPHeader dataHeader = createUDPHeader(clientPort, serverPort, ++sequenceNumber, ++acknowledgmentNumber, 0, bytesRead);
                if (sendDataAndWaitForAck(clientSocket, clientAddr, serverAddr, dataHeader, data, bytesRead) == true)
                {
                    setConsoleColor(10);
                    cout << "当前数据包传输完毕" << endl;
                    setConsoleColor(7);
...
int main() {
    ...
    if (s== "q") 
    {
        ...
        udpClient(clientIP, serverIP, serverPort, clientPort);
    }
    return 0;
}