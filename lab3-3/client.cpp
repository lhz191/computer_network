...
// 定义丢包率和延时
const int PACKET_DROP_RATE = 30; // 每 100 个包丢掉 PACKET_DROP_RATE 个
...
bool sendData(int clientSocket, const sockaddr_in& clientAddr, const sockaddr_in& serverAddr, std::vector<char>& fileContent, long fileSize, int size, uint16_t sourcePort, uint16_t destPort, uint32_t& sequenceNumber, uint32_t& acknowledgmentNumber) {
    ...
    while (begin < fileSize) {
        if (is_timeout) {
            ...
            is_timeout = false;
            setConsoleColor(12);
            cout << "检测到拥塞情况，将阈值重置为size/2，窗口大小重置为1" << endl;
            setConsoleColor(7);
            ssthresh = size / 2;
            //cout /*<< "当前的ssthresh：" << ssthresh */<< endl;
            size = 1;
            setConsoleColor(10);
            cout << "当前窗口大小：" << size << "当前的ssthresh：" << ssthresh << endl;
            setConsoleColor(7);
            exceed = 0;
        }
        ...
        if (ackReceived) {
            ...
            //setConsoleColor(7);
            if (ackNumber != ackHistory[ackCount - 1])
            {
                cout << "收到新的确认ack：" << ackNumber << endl;
                if (exceed)
                {
                    rtt_count++;
                    if ((middle - begin) / BUFFER_SIZE != 0) {
                        //cout << "rtt_count" << rtt_count << "要达到的count:" << (middle - begin) / BUFFER_SIZE << endl;
                    }
                    if (rtt_count >= (middle - begin) / BUFFER_SIZE && (middle - begin) / BUFFER_SIZE != 0)
                    {
                        size += 1;
                        rtt_count = 0;
                        setConsoleColor(10);
                        cout << "窗口resize" << endl;
                        cout << "当前窗口大小：" << size << endl;
                        cout << "当前状态：拥塞避免状态" << endl;
                        setConsoleColor(7);
                    }
                }
                else
            ...
            setConsoleColor(10);
            cout << "窗口前移，新的 begin: " << begin << ", end: " << end << endl;
            setConsoleColor(7);
            ...
            if (ackHistory[0] == ackHistory[1] && ackHistory[1] == ackHistory[2] && ackHistory[0] != 0) {
                ...
                {
                    setConsoleColor(12);
                    cout << "出现丢包情况，但拥塞不严重，将ssthresh重置为size/2，将窗口重置为 ssthresh + 3" << endl;
                    setConsoleColor(7);
                    ssthresh = size / 2;
                    //cout << "当前的ssthresh：" << ssthresh << endl;
                    size = size / 2 + 3;
                    setConsoleColor(10);
                    cout << "当前的ssthresh：" << ssthresh << "当前窗口大小：" << size << endl;
                    setConsoleColor(7);
                    exceed = 1;//进入拥塞避免阶段
                    rtt_count = 0;
                    setConsoleColor(10);
